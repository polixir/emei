...
class BaseInvertedDoublePendulumEnv(MujocoEnv, utils.EzPickle):
    def __init__(self,
        ...
        if self.sin_cos:
            self._causal_graph = np.array([[0, 0, 0, 0, 0, 0, 0, 0],  # x
                                           ...
                                           [0, 0, 0, 1, 1, 1, 1, 1]])  # action
        else:
            self._causal_graph = np.array([[0, 0, 0, 0, 0, 0],  # x
                                           [0, 0, 0, 1, 1, 1],  # theta1
                                           [0, 0, 0, 1, 1, 1],  # theta2
                                           [1, 0, 0, 0, 0, 0],  # v
                                           [0, 1, 0, 1, 1, 1],  # omega1
    ...
    def get_batch_obs(self, batch_state):
        if self.sin_cos:
            ...
            theta2 += theta1
            batch_obs = np.concatenate([qpos[:, :1],
                                        np.sin(theta1)[:, None], np.cos(theta1)[:, None],
                                        np.sin(theta2)[:, None], np.cos(theta2)[:, None],
                                        qvel], axis=-1)
        else:
    ...
    def get_batch_state(self, batch_obs):
        if self.sin_cos:
            ...
            batch_state = np.concatenate([batch_obs[:, :1], theta1[:, None], theta2[:, None], batch_obs[:, -3:]],
                                         axis=-1)
        else:
    ...
    def get_batch_init_state(self, batch_size):
        ...
        qvel = self._reset_noise_scale * self.np_random.standard_normal((batch_size, self.model.nv))
...
if __name__ == '__main__':
    ...
    env = TimeLimit(BoundaryInvertedDoublePendulumBalancingEnv(), max_episode_steps=1000, new_step_api=True)
