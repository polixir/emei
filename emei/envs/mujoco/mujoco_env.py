...
class EmeiMujocoEnv(EmeiEnv, MujocoEnv):
    def __init__(
            ...
            init_noise_params: Union[float, Dict[str, float]] = 0,
            obs_noise_params: Union[float, Dict[str, float]] = 0,
            # camera
    ...
    def build_noise_params(self, noise_params: Union[float, Dict[str, float]]):
    ...
    def additive_gaussian_noise(
            ...
            noise_params: Union[float, Dict[str, float]]
    ):
        """
        0:  free    7-pos   6-vel
        1:
        ...
        for jnt_id, jnt_type in enumerate(self.model.jnt_type):
            ...
            if isinstance(noise_params, dict):
                if jnt_id in noise_params:
                    pos_n, vel_n = noise_params["pos"] + np.zeros(noise_dim), noise_params["vel"] + np.zeros(noise_dim)
                else:
            ...
            if jnt_type == 0:
                ...
                noisy_pos[cur_pos_idx: cur_pos_idx + 3] += np.random.rand(batch_size, 3) * pos_n[:3]
                angle = Rotation.from_quat(origin_pos[cur_pos_idx + 3: cur_pos_idx + 7]).as_euler('zyx', degrees=True)
                angle += np.random.rand(batch_size, 3) * pos_n[3:6]
                noisy_pos[cur_pos_idx + 3: cur_pos_idx + 7] = Rotation.from_euler('xyz', angle, degrees=True).as_quat()

                noisy_vel[cur_vel_idx: cur_vel_idx + 3] += np.random.rand(batch_size, 3) * vel_n[:3]
                noisy_vel[cur_vel_idx + 3: cur_vel_idx + 6] += np.random.rand(batch_size, 3) * vel_n[3:6]
            elif jnt_type == 1:
                raise NotImplementedError
            else:
                pos_len, vel_len = 1, 1
                noisy_pos[cur_pos_idx: cur_pos_idx + 1] += np.random.rand(batch_size, 1) * pos_n
                noisy_vel[cur_vel_idx: cur_vel_idx + 1] += np.random.rand(batch_size, 1) * vel_n
