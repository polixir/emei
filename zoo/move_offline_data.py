...
FILE_NEEDED = [
    "uniform.h5",
    "SAC-random.h5",
    "SAC-random-agent.zip",
    "SAC-medium.h5",
    "SAC-medium-replay.h5",
    "SAC-medium-agent.zip",
    "SAC-expert.h5",
    "SAC-expert-replay.h5",
    "SAC-expert-agent.zip",
    "rollout_info.json",
]
DIR_NEEDED = ["tb"]
    ...
    for env in os.listdir(all_exp_dir):
        for params in os.listdir(os.path.join(all_exp_dir, env)):
            for time in os.listdir(os.path.join(all_exp_dir, env, params)):
                ...
                complete = set(file_needed) < set(contents) and set(dir_needed) < set(
                    contents
                )
                if complete:
                    ...
                    for file in file_needed:
                        shutil.copy(
                            os.path.join(source_data_dir, file),
                            os.path.join(target_data_dir, file),
                        )
                    for dir in dir_needed:
                        shutil.copytree(
                            os.path.join(source_data_dir, dir),
                            os.path.join(target_data_dir, dir),
                        )
